...
function TimelineCard({ item, index }: { item: Entry; index: number }) {
  return (
    <motion.div
    ...
    >
      <div className="flex flex-col items-center">
        <div
          className="w-3 h-3 rounded-full mt-6 shrink-0"
          style={{ background: item.periodColor }}
        />
        ...
        {loading ? (
          ...
          <div className="mb-20">
            {experiences.map((item, i) => (
              <TimelineCard key={item._id} item={item} index={i} />
            ))}
          </div>
        )}
